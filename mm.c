...
void *mm_realloc(void *oldptr, size_t size)
{
    ...
    { 
        size_t prev_alloc = is_allocated(prev(oldptr));
        size_t next_alloc = is_allocated(next(oldptr));
        size_t csize;
      
        if ((!next_alloc && ((oldsize + get_size(next(oldptr))) >= newsize)) || 
           (!prev_alloc && ((oldsize + get_size(prev(oldptr))) >= newsize)))
        {
            void *newptr;

            if (!next_alloc)
            {
                csize = oldsize + get_size(next(oldptr));
                delete_out_of_free_BST(next(oldptr));
            ...
            {
                csize = oldsize + get_size(prev(oldptr));
                delete_out_of_free_BST(prev(oldptr));
                
                newptr = prev(oldptr);
                memcpy(newptr, oldptr, oldsize - HEADFOOT);
            }
